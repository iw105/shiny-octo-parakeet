...
function handleInput(W, H) {
  G.players.forEach((p,i) => {
    ...
    if (k.arrow && p.cd.arrow<=0) {
      ...
      Body.setVelocity(arrow, {x:dir*ARROW_SPD + b.velocity.x*0.5, y:-1});
      World.add(G.engine.world, arrow);
      G.arrows.push({ body:arrow, ownerId:i, age:0 });
    }
...
function updateArrows(W, H) {
  G.arrows = G.arrows.filter(a=>{
    ...
    G.players.forEach((p,j)=>{
      ...
      if(Math.sqrt(dx*dx+dy*dy)<PLAYER_R+6){
        Body.applyForce(p.body,p.body.position,{x:a.body.velocity.x*0.003,y:-0.03});
        spawnParticles(a.body.position.x,a.body.position.y,'#ffe066',6);
        hit=true;
      }
    });
